...
// Resolves a single citation URL. Returns one of three outcomes, never
// throws -- a network failure is data (FAILED), not an exceptional
// condition the caller has to catch.
export async function resolveCitationUrl(url, { timeoutMs = DEFAULT_TIMEOUT_MS } = {}) {
  let host = null;
  try {
...
export async function resolveAllCitations(citations, opts = {}) {
  ...
  for (const c of citations || []) {
    const r = await resolveCitationUrl(c.url, opts);
...
// auto-discard an article over a host's bot defense. Only FAILED (a
// definitive non-200, unparseable URL, or network failure/timeout) trips.
export function evaluateCitationResolution(results) {
  ...
  return { tripped: failed.length > 0, failed, inconclusive, resolved, total: results.length };
}
