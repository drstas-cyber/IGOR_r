      ...
      ...stale.map(
        (f) =>
          `<li>PR <a href="${repoUrl}/pull/${f.number}">#${f.number}</a> — ${f.ageDays} дн. открыт — <code>${f.headRefName}</code><br>${
            f.isMarker
              ? '⛔ Это PR отклонённой попытки. <strong>Смержить</strong> — принять отклонение: маркер уходит в историю аудита, тема уходит на карантин до next_eligible_retry_at и вернётся сама. <strong>Закрыть</strong> без мержа — отменить отклонение: ничего не записывается, тема сразу возвращается в очередь. Оба варианта допустимы; смержить — обычное действие.'
